(function(){
  ...
  // The GIF previews are generated by a GitHub Action, not by this page —
  // a browser can't fetch the raw video off Drive (CORS), and a token with
  // rights to trigger the workflow can't live in client-side code on a
  // public repo. So this just links an admin to the workflow's own
  // "Run workflow" button, and reports how many clips are still waiting.
  const GIF_WORKFLOW_URL =
    'https://github.com/pinjulee-glitch/bachata-vocabulary/actions/workflows/generate-gifs.yml';

  function checkPendingGifs(){
    const el = document.getElementById('gifPending');
    if(!el) return;
    const ids = [];
    categories.forEach(c => c.moves.forEach(m => { if(m.driveId) ids.push(m.id); }));
    if(!ids.length){ el.textContent = ''; return; }
    Promise.all(ids.map(id =>
      fetch(`gifs/${id}.gif`, { method: 'HEAD' })
        .then(r => r.ok ? 0 : 1)
        .catch(() => 1)
    )).then(results => {
      const missing = results.reduce((a, b) => a + b, 0);
      el.textContent = missing
        ? `${missing} clip${missing === 1 ? '' : 's'} waiting for a GIF — click Generate GIFs, then "Run workflow"`
        : `All ${ids.length} clips have GIF previews`;
      el.classList.toggle('is-pending', !!missing);
