(function(){
  ...
  // `video` is optional and carries whichever of cloudinaryId / driveId /
  // videoUrl applies, so an admin can attach the clip as they add the move.
  function addMove(catId, title, note, video){
    const cat = categories.find(c => c.id === catId);
    if(!cat) return;
    const move = { id: uid('mv'), title: title.trim(), note: (note || '').trim() };
    if(video){
      if(video.cloudinaryId) move.cloudinaryId = video.cloudinaryId;
      if(video.driveId) move.driveId = video.driveId;
      if(video.videoUrl) move.videoUrl = video.videoUrl;
    }
    cat.moves.push(move);
    openCats.add(catId);
    persistStructure();
  ...
  // One tile for everyone. A member's clip lands in `submissions` as
  // pending and is visible only to them until an admin approves it; an
  // admin's goes straight into the shared library, since asking them to
  // approve their own would be busywork.
  function buildUploadTile(cat){
    const li = document.createElement('li');
    li.className = 'move upload-tile contributor-only';
    li.innerHTML = `
      <button type="button" class="add-trigger upload-trigger">
        ...
        <span>${isAdmin ? 'Add a move' : 'Upload a clip'}</span>
      </button>
    ...
    trigger.addEventListener('click', () => {
      if(!profile && !isAdmin){ openProfileForm(); return; }
    ...
    saveBtn.addEventListener('click', () => {
      ...
      const submit = (videoFields) => {
        if(isAdmin){
          addMove(cat.id, title, noteInput.value, videoFields);
          return Promise.resolve();
        }
        return Backend.addSubmission(Object.assign({
          title,
